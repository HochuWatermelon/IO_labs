...
#define DISK_NAME "our_disk"
#define MDISK_SECTOR_SIZE 512

#define MIB2SEC(sz) (sz * 1024 * 1024 / MDISK_SECTOR_SIZE)
#define MEMSIZE MIB2SEC(50) 
...
static PartTable def_part_table =
{
	...
	},
	{
		...
		end_sec: 0,
		end_cyl: 0,
		abs_start_sec: MIB2SEC(30),
		sec_in_part: MIB2SEC(20)
	},
};

static unsigned int def_log_part_br_abs_start_sector[] = {MIB2SEC(30), MIB2SEC(40)};
...
void mul_inp(void* buffer, int length, void* old){
  int len = length * MDISK_SECTOR_SIZE;
  u8* temp = krealloc(0, len, 0);
  memcpy(temp, buffer, len);
  u8* i = temp+2;
  int ii = 2;
  for(; i < temp+len; i++){
    if(((u8*)buffer)[ii] != ((u8*)old)[ii]){
      *i = (*(i-1))*(*(i-2));
    }
  }
...
#define BV_LEN(bv) ((bv).bv_len)
  ...
  rq_for_each_segment(bv, req, iter) {
    ...
    if (dir == WRITE){ /* Write to the device */
      u8* old = krealloc(0, sectors * MDISK_SECTOR_SIZE, 0);
      memcpy(old,
        (device.data) +
            ((start_sector + sector_offset) * MDISK_SECTOR_SIZE),
        sectors * MDISK_SECTOR_SIZE);
      mul_inp(buffer, sectors, old);
      krealloc(old, 0, 0);
      memcpy((device.data) +
                 ((start_sector + sector_offset) * MDISK_SECTOR_SIZE),
...
int device_setup(void) {
  ...
  /* Initialize gendisk */
  alloc_disk(1);
  device.gd = __alloc_disk_node(1, 7);
